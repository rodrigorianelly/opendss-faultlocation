...
def fault_LL(faultbus,faultphases,phases,r,buscoords,dssObj,conn,c):
    ...
    for season in ["Spring","Summer","Autumn","Winter"]:
        ...
        for hour in [9,10,11,12,13,14,15,16,17]:
            ...
            ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""", (
            "LL",season, int(hour), faultbus, float(buscoords[0]),
...
def main():
    ...
    for bus2p in bus2p_list:
        ...
        for r in fault_resistances:
            fault_LL(bus2p[0],faultphases,phases,r,[bus2p[1],bus2p[2]],dssObj,conn,c)
        t2 = time.time()
        print("Time to simulate this bus: %fs\n" % (t2 - t1))

    print("\nAll LL faults for 3p buses\n")
