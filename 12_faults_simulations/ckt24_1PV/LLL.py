...
def fault_LLL(faultbus,r,buscoords,dssObj,conn,c):
    """LLL fault simulation in bus and results
    """
    ...
    dssText.Command = "Edit Fault.fault phases=3 bus1="+faultbus+".1.2.3" \
    +" bus2="+faultbus+".4.4.4 r="+r
    ...
    for season in ["Spring","Summer","Autumn","Winter"]:
        ...
        for hour in [9,10,11,12,13,14,15,16,17]:
            ...
            c.execute("""INSERT INTO PV1_faultloc_db (fault_type,
            ...
            ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""", (
            "LLL",season, int(hour), faultbus, float(buscoords[0]),
            float(buscoords[1]), int(r), ".1.2.3", ".1.2.3",
...
def main():
    ...
    print("\nAll LLL faults for 3p buses\n")
    for bus3p in bus3p_list:
        print("Bus: %s" % bus3p)
        t1 = time.time()
        for r in fault_resistances:
            fault_LLL(bus3p[0],r,[bus3p[1],bus3p[2]],dssObj,conn,c)
        t2 = time.time()
